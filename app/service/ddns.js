...
const DDNS = async (client, target, cb) => {
  ...
  const DOMAINNAME = SUBDOMAIN.split('.')
    .slice(-2)
    .join('.');
  const RR = SUBDOMAIN.split('.')
    .slice(0, -2)
    .join('.');
  const updateParmas = {
  ...
  await client.request(describeSubParams.Action, describeSubParams, requestOption)
    .then((result) => {
  ...
  if (shouldUpdate) {
    return client.request(updateParmas.Action, updateParmas, requestOption)
      .then((resolve, reject) => {
  ...
  if (shouldAdd) {
    return client.request(addParmas.Action, addParmas, requestOption)
      .then((result) => {
